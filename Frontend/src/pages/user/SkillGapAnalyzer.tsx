...
const SkillGapAnalyzer: React.FC = () => {
  ...
  // New primary color variables
  const primaryColor = '#21589C';
  const lightBlueBg = 'bg-[#F0F5FA]'; // A light shade for containers
  const lightBlueBorder = 'border-[#D1E0F0]'; // A light shade for borders
    ...
    <Layout role="student">
      <div className="p-6 bg-gray-50 min-h-screen">
        <div className="max-w-7xl mx-auto">
          {/* Upload Section */}
          <div className={`${lightBlueBg} rounded-xl ${lightBlueBorder} p-8 flex flex-col md:flex-row items-center justify-between mb-6`}>
            <div className="flex-1 flex flex-col items-center md:items-start">
              <Upload className={`h-12 w-12 text-[${primaryColor}] mb-4`} />
                    ...
                    <div className="flex flex-col gap-3 mb-4">
                      {['Senior Frontend Developer', 'Backend Developer', 'Full Stack Developer'].map((role) => (
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg font-medium text-sm ${targetRole === role ? `bg-[${primaryColor}] text-white` : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                        >
                          {role}
                        </button>
                      ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 text-center">
                  ...
                  <div className="relative flex items-center justify-center mb-2">
                    <svg className="w-24 h-24" viewBox="0 0 36 36">
                      ...
                      <path d={`M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831`} fill="none" stroke={primaryColor} strokeWidth="3" strokeDasharray={`${readinessScore}, 100`} />
                    </svg>
                    <span className={`absolute text-2xl font-bold text-[${primaryColor}]`}>{readinessScore}%</span>
                  </div>
                  ...
                  <div className={`text-right text-xs text-[${primaryColor}]`}>Market Demand: <span className="font-bold">Very High</span></div>
                </div>
                ...
                <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <span className={`font-medium text-[${primaryColor}] text-base`}>{targetRole}</span>
                    <span className="font-bold text-green-700 text-base">₹15-25L</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span>Market Demand:</span>
                    <span className={`bg-[${lightBlueBg}] text-[${primaryColor}] px-2 py-1 rounded-full text-xs font-medium`}>Very High</span>
                  </div>
                  <button 
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="flex border-b border-gray-200 bg-gray-100">
                  <div className="flex-1 flex">
                    <button 
                      ...
                      className={`px-6 py-3 text-sm font-medium ${activeTab === 'skill-gap' ? `bg-white text-[${primaryColor}]` : 'text-gray-600 hover:text-gray-800'}`}
                    >
                      Skill Gap
                    </button>
                    <button 
                      onClick={() => setActiveTab('skills-have')}
                      className={`px-6 py-3 text-sm font-medium ${activeTab === 'skills-have' ? `bg-white text-[${primaryColor}]` : 'text-gray-600 hover:text-gray-800'}`}
                    >
                      Skills I Have
                    </button>
                    <button 
                      onClick={() => setActiveTab('recommendations')}
                      className={`px-6 py-3 text-sm font-medium ${activeTab === 'recommendations' ? `bg-white text-[${primaryColor}]` : 'text-gray-600 hover:text-gray-800'}`}
                    >
                      Recommendations
                    </button>
                    <button
                      onClick={() => setActiveTab('trending-skills')}
                      className={`px-6 py-3 text-sm font-medium ${activeTab === 'trending-skills' ? `bg-white text-[${primaryColor}]` : 'text-gray-600 hover:text-gray-800'}`}
                    >
                      Trending Skills
                    </button>
                  </div>
                ...
                <div className="p-6">
                  ...
                  {activeTab === 'recommendations' && (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                      <div className="flex items-center mb-4">
                        <BookOpen className={`h-6 w-6 text-[${primaryColor}] mr-2`} />
                      ...
                      <div className="space-y-3">
                        {[
                          ...
                          <div key={index} className={`flex items-center h-16 ${lightBlueBg} ${lightBlueBorder} rounded-full overflow-hidden mb-2`}>
                            <div className="flex-1 flex flex-col pl-4">
                              <span className="font-medium text-sm">{course.course}</span>
                              <span className="text-xs text-gray-600">For: <span className={`bg-[${lightBlueBg}] text-[${primaryColor}] px-2 py-1 rounded`}>{course.skill}</span> • {course.provider} • {course.duration} • <span className="text-yellow-400">★</span>{course.rating}</span>
                            </div>
                            <div className="text-right pr-4 flex flex-col items-end">
                              <span className={`font-semibold text-[${primaryColor}] text-sm`}>{course.price}</span>
                              <button className={`px-3 py-1 bg-[${primaryColor}] text-white rounded text-xs hover:bg-opacity-80 mt-1`}>Enroll Now</button>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {jobRecommendations.map((job, index) => (
                      <div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md">
                        ...
                        <div className="flex items-center justify-between">
                          <span className={`text-sm font-medium text-[${primaryColor}]`}>Match: {job.match}</span>
                          <button className={`text-[${primaryColor}] text-sm hover:underline`} onClick={() => setSelectedJob(job)}>View Details</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button 
                    ...
                    className={`px-4 py-2 bg-[${primaryColor}] text-white rounded-lg text-sm hover:bg-opacity-80`}
                  >
                    Submit
                  </button>
                </div>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-[${primaryColor}] focus:border-[${primaryColor}]`}
                />
                <div className="mt-4">
                  <button 
